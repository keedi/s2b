...
my %url   = (
    category_list => 'http://www.s2b.kr/S2BNCustomer/S2B/scrweb/remu/rema/searchengine/s2bCustomerSearch.jsp',
    detail        => 'http://www.s2b.kr/S2BNCustomer/rema100No.do',
);
...
my $category_info = get_category();
my @codes         = get_searched_product_codes( $model, $count );
my %info;
for my $code (@codes) {
    warn "  code: $code\n";
    warn "    fetching from the web...\n";

    $info{$code} = get_item($code);
}

warn "sorting via price...\n";
...
for my $code (@sorted_codes) {
    ...
    #
    warn "    saving images...\n";
    save_images( $model, $code, @{ $i->{images} } );

    #
    # save csv
    #
    #my $c1 = get_code( $category, 1 );
    #my $c2 = get_code( $category, 2 );
    ...
    $csv->print(
        ...
        [
            ...
            $i->{c2},
            $i->{c3},
            $category_info->{v}{ $i->{c1} },
            $category_info->{v}{ $i->{c2} },
            $category_info->{v}{ $i->{c3} },
            $i->{name},
            $i->{model},
            $i->{price},
            $i->{manufactory},
...
sub get_searched_product_codes {
    my $query = shift;
    my $count = shift || 10;
...
sub save_images {
    my ( $category, $code, @urls ) = @_;

    for ( my $i = 0; $i < @urls; ++$i ) {
        my $res = $http->get( $urls[$i] );
...
sub get_url {
    ...
    return $url{$type} . '?' . HTTP::Tiny->www_form_urlencode($params),
}
...
sub get_category {
    ...
    while (<DATA>) {
        ...
        $category{k}{$code} = $name;
        $category{v}{$name} = $code;
    }
...
http://www.s2b.kr/S2BNCustomer/rema100No.do?forwardName=detail&f_re_estimate_code=201208220545321

{
    expiration_date     undef,
    images              [
        [0] "http://www.s2b.kr:80/S2BNfiledata/remu/2011101230305/rema/S2B_AA_2011101230305_1345626634421.jpg"
    ],
    made_in             "일본",
    manufactory         "CANON",
    manufactured_date   "20110706 /",
    model               "캐논 점품토너 NPG-51 (IR-2520/2525/2530)",
    price               70000,
    spec                "IR 2520,IR 2520i,IR",
    tax                 "과세유무",
    묶음배송여부              "가능",
    물품목록번호              "44103103-22310516",
    '물품정보 유효기간'         "2014-02-14",
    배송방법                "택배",
    배송비                 "무료",
    세부품명번호              "-",
    '소재 / 재질'           "플라스틱"
